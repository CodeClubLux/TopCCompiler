...
model_ModelRenderer_set_materials(main_mesh_renderer,tmpcameraMb(&(main_cube_materials)),f);
...
model_ModelRenderer_set_materials(main_model_renderer,tmpcameraMb(&(main_cube_materials)),f);
;}
struct fpsController_FPSController* main_fps_controller;struct lights_DirLight* main_main_light;struct ibl_Skybox* main_skybox;_Bool _global_Maybe_is_none_rlister_NameByValue(struct lister_Name* _global_self, struct _global_Context* f){;
...
main_cube_materials = _global_StaticArray_1_materialSystem_MaterialInit(materialSystem_MaterialInit(_global_StringInit(15,"DefaultMaterial"),shader_Shader_get_id(materialSystem_pbr_shader,(&_global_context)),tmpmaterialSystembk(&(main_wet_street_params)),&(draw_default_state)));;
main_fps_controller = fpsController_make_FPSController(ecs_Store_id_of_camera_Camera(camera_cameras,((&_global_context))->camera,(&_global_context)),(&_global_context));;
bowWeapon_make_Bow(ecs_Store_id_of_camera_Camera(camera_cameras,((&_global_context))->camera,(&_global_context)),(&_global_context));
